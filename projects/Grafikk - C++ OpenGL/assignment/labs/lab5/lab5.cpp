...
Lab5::Lab5(std::string name, std::string version) : GLFWApplication(name, version)
{
...
unsigned Lab5::Run() {
    ...
    textureManager->LoadTexture2DRGBA("chess/Texture", texturesDir, 0);
    chessboardShader->Bind();
    chessboardShader->UploadUniformInt("uTexture", 0);
    float newAmbientStrength = 1.0f;
    chessboardShader->UploadUniformFloat("u_ambientStrength", newAmbientStrength);
    ...
    glm::vec3 lightColor =glm::vec3(1.0f, 1.0f, 1.0f); // Set light color to white
    ...
    cubeShader->UploadUniformFloat("u_ambientStrength", newAmbientStrength);
    cubeShader->UploadUniformFloat3("u_lightSourcePosition", cameraPosC);
    cubeShader->UploadUniformFloat3("u_lightColor", lightColor);

    cubeShader->UploadUniformFloat3("u_cameraPosition", cameraPosC);
    ...
    bool ambiemntswitch = false;
    while (!glfwWindowShouldClose(window)) {
        RenderCommands::Clear();
        if (!ambiemntswitch) {
            newAmbientStrength = newAmbientStrength - 0.001f;
            if (newAmbientStrength <= 0) {
                ambiemntswitch = true;
            }
        }else {
            newAmbientStrength = newAmbientStrength + 0.001f;
            if (newAmbientStrength >= 1) {
                ambiemntswitch = false;
            }
        }
...
void input(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_A && state == GLFW_PRESS) { keys[4] = 1; }

    state = glfwGetKey(window, GLFW_KEY_W);
    if (key == GLFW_KEY_W && state == GLFW_PRESS) { keys[5] = 1; }

    state = glfwGetKey(window, GLFW_KEY_S);
    if (key == GLFW_KEY_S && state == GLFW_PRESS) { keys[6] = 1; }

    state = glfwGetKey(window, GLFW_KEY_D);
    if (key == GLFW_KEY_D && state == GLFW_PRESS) { keys[7] = 1; }
